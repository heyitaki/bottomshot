...
export const getUserMomentListingsDedicated = (
  ...
  return axios
    .post('https://api.nbatopshot.com/marketplace/graphql?GetUserMomentListingsDedicated', {
    ...
    .then((res) => {
      if (res.status === 200) {
        return res.data as GetUserMomentListingsDedicatedRes;
      }

      throw `Failed to fetch data from Topshot servers for set ID ${setId} and play ID ${playId}.`;
    });
};
